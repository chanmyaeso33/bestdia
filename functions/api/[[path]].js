...
async function adminOrders(request, env) {
  ...
  const repairedOrders = await Promise.all(activeOrders.map(async (order) => {
    ...
    const isBestDiaBalance = data.payKey === "balance" || data.payment === "BestDia Balance";
    const mxStatus = data.mxshopTopup?.status || "";
    if (!isBestDiaBalance || data.status === "cancelled" || mxStatus === "success" || mxStatus === "failed" || mxStatus === "skipped") return order;
    const now = new Date().toISOString();
