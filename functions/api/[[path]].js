...
const DEFAULT_MXSHOP_SUBMITTED_AUTO_COMPLETE_SECONDS = 90;
const PAYMENTS = {
...
async function createOrder(request, env) {
  ...
  if (trustedPay.key === "balance") {
    ...
    if (autoTopup.status === "success") {
      ...
      updates.completedAt = autoTopup.completedAt || topupNow;
      timeline.push({ status: "completed", time: Date.now() });
    } else {
      updates.status = "processing";
    }
    await updateOrderDoc(firestore, created.docId, updates);
  }
  ...
  return jsonResponse(200, { ok: true, orderId, docId: created.docId, notification, autoTopup });
}
...
async function adminOrders(request, env) {
  ...
  const repairedOrders = await Promise.all(activeOrders.map(async (order) => {
    ...
    if (isBestDiaBalance && isSubmittedLikeMxTopup(data.mxshopTopup) && shouldAutoCompleteSubmittedMx(data, env)) {
      const now = new Date().toISOString();
      ...
      const updated = await updateOrderDoc(firestore, order.docId, {
        ...
        mxshopTopup: {
          ...
          autoCompleted: true,
          autoCompletedAt: now,
          reason: data.mxshopTopup?.reason || data.mxshopTopup?.error || `Auto-completed after MXShop grace period`,
        },
        updatedAt: now,
        timeline,
      });
...
async function tryAutoFulfillMxshopOrder(env, order) {
  ...
  try {
    ...
    const now = new Date().toISOString();
    return { status: topup.status || (topup.skipped ? "skipped" : "submitted"), skipped: Boolean(topup.skipped), reason: topup.reason || "", request: topup.request || null, response: topup.response || null, completedAt: topup.status === "success" ? now : "", submittedAt: topup.status === "submitted" ? now : "" };
  } catch (error) {
...
async function performMxshopTopup(env, order) {
  ...
  return { skipped: false, status: isMxDeliveryConfirmed(data) ? "success" : "submitted", response: data || text, request: requestBody };
}

function isMxDeliveryConfirmed(data) {
  if (hasMxPendingSignal(data) || hasMxFailureSignal(data)) return false;
  const direct = [
...
function shouldAutoCompleteSubmittedMx(order, env = {}) {
  const topup = order?.mxshopTopup || {};
  if (!isSubmittedLikeMxTopup(topup)) return false;
  const rawSeconds = Number(env.MXSHOP_SUBMITTED_AUTO_COMPLETE_SECONDS ?? DEFAULT_MXSHOP_SUBMITTED_AUTO_COMPLETE_SECONDS);
  const delaySeconds = Number.isFinite(rawSeconds) ? Math.max(0, rawSeconds) : DEFAULT_MXSHOP_SUBMITTED_AUTO_COMPLETE_SECONDS;
  const submittedAt = topup.submittedAt || topup.completedAt || topup.failedAt || topup.startedAt || order.updatedAt || order.createdAt;
  const submittedTime = submittedAt ? new Date(submittedAt).getTime() : 0;
  return submittedTime > 0 && Date.now() - submittedTime >= delaySeconds * 1000;
}

function isSubmittedLikeMxTopup(topup) {
  if (!topup || typeof topup !== "object") return false;
  if (topup.status === "submitted" || topup.status === "sending") return true;
  if (topup.status === "failed" && hasMxPendingSignal(topup)) return true;
  return false;
}
