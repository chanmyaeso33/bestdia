...
const PRODUCTS = (() => {
  ...
  mlbb.packages = [
    ...
    { id: 5, title: "Weekly Elite Bundle", name: "Bundle", supplierPriceThb: 25.74 },
    { id: 6, title: "Monthly Epic Bundle", name: "Bundle", supplierPriceThb: 127.92 },
  ...
  pubg.packages = [
  ...
  ].map(([uc, thb]) => withPrice({ id: `pubg-${uc}`, title: `${uc} UC`, name: "", uc: Number(uc), supplierPriceThb: thb }, pubg)).concat([
    { id: "pubg-first-purchase-pack", title: "First Purchase Pack", name: "Special Pack", supplierPriceThb: 32.10 },
    { id: "pubg-prime-1-month", title: "Prime 1 Month", name: "Subscription", supplierPriceThb: 33.01 },
    { id: "pubg-firearm-materials-pack", title: "Upgradable Firearm Materials Pack", name: "Materials Pack", supplierPriceThb: 89.10 },
    ...
    { id: "pubg-prime-plus-1-month", title: "Prime Plus 1 Month", name: "Subscription", supplierPriceThb: 299.01 },
...
async function createOrder(request, env) {
  ...
  const cleanOrder = {
    ...
    status: trustedPay.key === "balance" ? "processing" : "pending",
    createdAt: now,
    updatedAt: now,
    timeline: [{ status: trustedPay.key === "balance" ? "processing" : "pending", time: Date.now() }],
  };

  const firestore = await firestoreClient(env);
  if (trustedPay.key === "balance") {
    const accountId = normalizeAccountId(cleanOrder.accountId);
    if (!accountId) return jsonResponse(400, { ok: false, error: "Missing BestDia account ID" });
    const account = await getAccountBalanceDoc(firestore, accountId);
    const price = Math.ceil(Number(trustedPkg.price || 0) / THB_TO_KS);
    const balance = Number(account?.balance || 0);
    if (balance < price) return jsonResponse(400, { ok: false, error: "BestDia balance is not enough" });
    const history = Array.isArray(account?.history) ? [...account.history] : [];
    history.unshift({ type: "purchase", title: `Paid ${packageText(trustedPkg)}`, amount: -price, orderId, at: now });
  ...
  if (trustedPay.key === "balance") {
    const autoTopup = await tryAutoFulfillMxshopOrder(env, cleanOrder);
    if (autoTopup) {
      const updates = {
        mxshopTopup: autoTopup,
        updatedAt: new Date().toISOString(),
      };
      if (autoTopup.status === "success" || autoTopup.status === "skipped") {
        updates.status = autoTopup.status === "success" ? "completed" : "processing";
        updates.timeline = [...cleanOrder.timeline, { status: updates.status, time: Date.now() }];
      }
      const updatedOrder = await updateOrderDoc(firestore, created.docId, updates);
      Object.assign(cleanOrder, updatedOrder);
    }
  }
  ...
  return jsonResponse(200, { ok: true, orderId, docId: created.docId, notification });
}
...
async function adminOrders(request, env) {
  ...
  const firestore = await firestoreClient(env);
  const orders = await listOrders(firestore);
  return jsonResponse(200, { ok: true, orders: orders.filter((order) => !order.data.archived).map(adminOrder) });
}
...
async function adminUpdateOrder(request, env) {
  ...
  if (payload.mxshopTopup && typeof payload.mxshopTopup === "object") updates.mxshopTopup = payload.mxshopTopup;
...
async function mxshopTopup(request, env) {
  ...
  try {
    ...
    return jsonResponse(200, { ok: true, skipped: Boolean(topup.skipped), reason: topup.reason || "", mxshop: topup.response || null, request: topup.request || null });
  } catch (error) {
...
async function tryAutoFulfillMxshopOrder(env, order) {
  ...
  try {
    ...
    return { status: topup.skipped ? "skipped" : "success", skipped: Boolean(topup.skipped), reason: topup.reason || "", request: topup.request || null, response: topup.response || null, completedAt: new Date().toISOString() };
  } catch (error) {
...
async function performMxshopTopup(env, order) {
  ...
  return { skipped: false, response: data || text, request: requestBody };
}
