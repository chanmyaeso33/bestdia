...
async function collectNewsApi(request, env) {
  ...
  const limitPerSource = Math.max(1, Math.min(20, Number(payload.limitPerSource || 5)));
  const enabledSources = NEWS_COLLECTOR_SOURCES.filter((source) => {
    if (!payload.game) return true;
    return source.game === normalizeOpportunityGame(payload.game);
  });
  const results = [];
