...
async function adminOrders(request, env) {
  ...
  const repairedOrders = await Promise.all(activeOrders.map(async (order) => {
    ...
    if (isBestDiaBalance && mxStatus === "success" && !isMxDeliveryConfirmed(data.mxshopTopup?.response)) {
      ...
      const updatedTopup = { ...data.mxshopTopup, status: "submitted", completedAt: "" };
...
async function tryAutoFulfillMxshopOrder(env, order) {
  ...
  try {
    ...
    return { status: topup.status || (topup.skipped ? "skipped" : "submitted"), skipped: Boolean(topup.skipped), reason: topup.reason || "", request: topup.request || null, response: topup.response || null, completedAt: topup.status === "success" ? new Date().toISOString() : "" };
  } catch (error) {
