...
function opportunitySchema() {
  return {
    type: "object",
    additionalProperties: false,
    required: ["should_create_opportunity", "title", "description", "game", "opportunity_type", "trend_score", "sales_score", "urgency_score", "myanmar_interest_score", "overall_score", "reasoning", "recommended_channels", "product_matches"],
    properties: {
      should_create_opportunity: { type: "boolean" },
      title: { type: "string" },
      description: { type: "string" },
      game: { type: "string", enum: ["mlbb", "pubg", "other"] },
      opportunity_type: { type: "string", enum: Array.from(OPPORTUNITY_TYPES) },
      ...
      myanmar_interest_score: { type: "integer", minimum: 0, maximum: 100 },
      overall_score: { type: "integer", minimum: 0, maximum: 100 },
      reasoning: { type: "string" },
      recommended_channels: { type: "array", items: { type: "string", enum: ["facebook", "tiktok", "instagram", "telegram", "website"] } },
      product_matches: {
        type: "array",
        items: {
          type: "object",
          additionalProperties: false,
          required: ["product_name", "relevance_score", "reason"],
          properties: {
...
async function processOpportunityArticle(env, article, products) {
  ...
  try {
    ...
    const opportunity = validateOpportunityResult(openai.result);
    ...
    const productMatches = matchOpportunityProducts(opportunity.product_matches, products, opportunity.game);
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      text: {
        format: {
          type: "json_schema",
          name: "bestdia_opportunity_agent_v1",
...
function validateOpportunityResult(result) {
  if (!result || typeof result !== "object") throw new Error("Opportunity result must be an object");
  if (!OPPORTUNITY_GAMES.has(result.game)) throw new Error("Invalid game returned by Opportunity Agent");
  if (!OPPORTUNITY_TYPES.has(result.opportunity_type)) throw new Error("Invalid opportunity_type returned by Opportunity Agent");
  return {
    ...
    recommended_channels: Array.isArray(result.recommended_channels) ? result.recommended_channels.map(String) : [],
    product_matches: Array.isArray(result.product_matches) ? result.product_matches : [],
  };
}

function matchOpportunityProducts(productMatches, products, game) {
  ...
  return productMatches.map((match) => {
    const product = productByName.get(String(match.product_name || "").trim().toLowerCase());
    if (!product || (game !== "other" && product.game !== game)) return null;
    return { product, relevance_score: normalizeOpportunityScore(match.relevance_score), reason: String(match.reason || "").trim() };
  }).filter(Boolean);
}
