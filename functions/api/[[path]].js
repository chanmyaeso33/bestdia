...
async function createOrder(request, env) {
  ...
  if (!String(order.payment || "").trim()) return jsonResponse(400, { ok: false, error: "Missing payment method" });

  const now = new Date().toISOString();
  const cleanOrder = {
    id: orderId,
    gameKey: String(order.gameKey || "").trim().slice(0, 32),
    gameName: String(order.gameName || "").trim().slice(0, 80),
    userId,
    zoneId: String(order.zoneId || "").trim().slice(0, 80),
    contact,
    ign: String(order.ign || "").trim().slice(0, 120),
    pkg,
    payment: String(order.payment || "").trim().slice(0, 40),
    payKey: String(order.payKey || "").trim().slice(0, 30),
...
async function publicTicker(request, env) {
  ...
  const items = orders
    .filter((order) => !order.data.archived && order.data.pkg)
    .slice(0, 10)
...
async function mxshopTopup(request, env) {
  ...
  if (!stockReleaseId) {
    return jsonResponse(400, {
      ok: false,
      error: `No MXShop stockreleaselist_id mapped for package id ${pkgId}. Add the supplier stockreleaselist_id to MXSHOP_PACKAGE_MAP, for example {"7":"YOUR_WEEKLY_PASS_ID"}, or set mxshopStockReleaseId on the package.`,
    });
  }
...
function publicTickerItem(order, index) {
  const suffix = String(order.id || order.userId || index + 1).replace(/\D/g, "").slice(-4) || String(index + 1).padStart(2, "0");
  const ign = String(order.ign || "").trim();
  const name = /^[^<>{}[\]\\]{2,24}$/.test(ign) && !/not\s*verified/i.test(ign) ? ign : `Player${suffix}`;
  const pkg = order.pkg || {};
  ...
  return `${name} bought ${packageName}`;
}
