...
async function createOrder(request, env) {
  ...
  const now = new Date().toISOString();
  const initialStatus = trustedPay.key === "balance" ? "completed" : "pending";
  const cleanOrder = {
    id: orderId,
    ...
    createdAt: now,
    updatedAt: now,
    completedAt: trustedPay.key === "balance" ? now : "",
...
async function adminOrders(request, env) {
  ...
  const repairedOrders = await Promise.all(activeOrders.map(async (order) => {
    ...
    if (!isBestDiaBalance || data.status === "completed" || data.status === "cancelled") return order;
    const now = new Date().toISOString();
