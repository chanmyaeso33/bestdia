...
async function productsWithSupplierPrices(env) {
  const supplierPrices = await getSupplierPrices(env);
...
async function getSupplierPrices(env) {
  if (!env.MXSHOP_MX_KEY || !env.MXSHOP_PASSKEY) return { prices: {}, updated: false };
  const prices = {};
  for (const stockIDX of parseMxStockIds({}, env)) {
    const response = await mxPost(env, "/api/v1/get_stockreleaselist", { StockIDX: stockIDX, type: 4 });
    for (const item of Array.isArray(response.result) ? response.result : []) {
      const id = String(item.stockreleaselist_id || "").trim();
      const price = Number(String(item.price ?? "").replace(/,/g, ""));
      if (id && Number.isFinite(price) && price > 0) prices[id] = price;
    }
  }
...
async function createBalanceTopup(request, env) {
  ...
  const sessionAccountId = await requireAccountSession(request, env);
  if (sessionAccountId instanceof Response) return sessionAccountId;
  if (sessionAccountId !== accountId) return jsonResponse(403, { ok: false, error: "You can only submit a top up for your own account" });
  ...
  const priorTopups = await listCollectionDocs(firestore, "balance_topups");
  const reviewRequired = priorTopups.some(({ data }) =>
    (transactionReference && transactionReference === String(data.transactionReference || "")) ||
    slipFingerprint === String(data.slipFingerprint || "")
  );
  const cleanTopup = {
    id: topupId,
    accountId,
    ...
    balanceCreditStatus: "not_credited",
...
async function accountBalance(request, env) {
  ...
  const sessionAccountId = await requireAccountSession(request, env);
  if (sessionAccountId instanceof Response) return sessionAccountId;
  if (sessionAccountId !== accountId) return jsonResponse(403, { ok: false, error: "Account access denied" });
  const firestore = await firestoreClient(env);
  const account = await getAccountBalanceDoc(firestore, accountId);
  return jsonResponse(200, { ok: true, exists: Boolean(account), accountId, balance: Number(account?.balance || 0), balanceStatus: account?.balanceStatus || "active", history: account?.history || [] });
}
...
async function accountSignup(request, env) {
  ...
  return jsonResponse(200, { ok: true, account: await publicAccount(firestore, profile), session: await createAccountSession(profile.accountId, env) });
}
...
async function createOrder(request, env) {
  ...
  if (trustedPay.key === "balance") {
    ...
    const sessionAccountId = await requireAccountSession(request, env);
    if (sessionAccountId instanceof Response) return sessionAccountId;
    if (sessionAccountId !== balanceAccountId) return jsonResponse(403, { ok: false, error: "Balance account access denied" });
    balanceAccount = await getAccountBalanceDoc(firestore, balanceAccountId);
    if (balanceAccount?.balanceStatus === "frozen") return jsonResponse(423, { ok: false, error: "Your BestDia Balance is temporarily unavailable while we verify your payment." });
  ...
  if (trustedPay.key === "balance") {
    ...
    if (autoTopup.status === "success") {
      ...
      await commitOrderAndAccountUpdates(firestore, created.docId, orderUpdates, balanceAccountId, accountUpdates, {
        transactionId: `purchase_${created.docId}`,
        userId: balanceAccountId, type: "purchase", amount: -balanceCharge, currency: "BD",
        balanceBefore: Number(balanceAccount?.balance || 0), balanceAfter: accountUpdates.balance,
        referenceType: "order", referenceId: orderId, description: `Paid ${packageText(trustedPkg)}`,
        createdBy: balanceAccountId, createdAt: topupNow,
      });
...
async function adminOpportunities(request, env) {
  ...
  const game = payload.game ? normalizeOpportunityGame(payload.game) : "";
...
async function adminUpdateOpportunity(request, env) {
  ...
  const id = String(payload.id || "").trim();
  const status = String(payload.status || "").trim();
  if (!id) return jsonResponse(400, { ok: false, error: "Missing opportunity ID" });
  if (!["new", "reviewed", "approved", "rejected", "used", "expired"].includes(status)) {
    return jsonResponse(400, { ok: false, error: "Invalid opportunity status" });
  }
  const rows = await supabaseRequest(env, "PATCH", `opportunities?${supabaseQuery({ id: `eq.${id}` })}`, {
    status,
    updated_at: new Date().toISOString(),
  ...
  let review = null;
  let reviewWarning = "";
  const reviewRow = buildOpportunityReviewRow(id, status, payload);
  if (reviewRow) {
    try {
      const reviewRows = await supabaseRequest(env, "POST", "opportunity_reviews", [reviewRow], "return=representation");
      review = reviewRows[0] || null;
    } catch (error) {
      reviewWarning = String(error.message || "Opportunity review was not saved").slice(0, 500);
    }
  }
...
async function adminConfirmBalanceTopup(request, env) {
  ...
  const current = await getCollectionDoc(firestore, "balance_topups", docId);
  if (!current) return jsonResponse(404, { ok: false, error: "Top up not found" });
  if (current.balanceCreditStatus === "credited" || current.verificationStatus === "verified") return jsonResponse(409, { ok: false, error: "Payment already processed." });
  if (!["pending", "verifying"].includes(current.verificationStatus || current.status)) return jsonResponse(400, { ok: false, error: "Only pending or verifying payments can be credited" });
  if (current.expiresAt && new Date(current.expiresAt).getTime() < Date.now()) return jsonResponse(400, { ok: false, error: "Expired payment cannot be verified normally" });
  ...
  history.unshift({ type: "topup", title: `Verified payment ${current.id}`, amount, topupId: current.id, at: new Date().toISOString() });
  const balance = Number(account?.balance || 0) + amount;
  const verifiedAt = new Date().toISOString();
  const updated = {
    ...current, status: "verified", verificationStatus: "verified", verifiedBy: "admin", verifiedAt,
    balanceCreditStatus: "credited", balanceCreditAmount: amount, creditedAmount: amount, balanceAfter: balance,
    timeline: [...(Array.isArray(current.timeline) ? current.timeline : []), { status: "verified", time: Date.now() }], updatedAt: verifiedAt,
  };
  try {
    await commitBalanceTopupVerification(firestore, docId, updated, accountId, {
      accountId, balance, balanceStatus: account?.balanceStatus || "active", history: history.slice(0, 40), updatedAt: verifiedAt,
    }, {
      transactionId: `topup_${docId}`, userId: accountId, type: "topup", amount, currency: "BD",
      balanceBefore: Number(account?.balance || 0), balanceAfter: balance, referenceType: "payment", referenceId: current.id || docId,
      description: `Verified payment ${current.id || docId}`, createdBy: "admin", createdAt: verifiedAt,
    });
  } catch (error) {
    if (/ALREADY_EXISTS|already exists/i.test(error.message || "")) return jsonResponse(409, { ok: false, error: "Payment already processed." });
    throw error;
  }
  await writeAuditLog(firestore, "admin", "payment_verified_and_credited", current.id || docId, current, updated, "");
...
async function adminSetBalanceStatus(request, env) {
  if (request.method !== "POST") return jsonResponse(405, { ok: false, error: "Method not allowed" });
  const payload = await readJson(request); const auth = requireAdmin(payload, env); if (auth) return auth;
  const accountId = normalizeAccountId(payload.accountId); const balanceStatus = String(payload.balanceStatus || "");
  if (!accountId || !["active", "frozen"].includes(balanceStatus)) return jsonResponse(400, { ok: false, error: "Invalid balance status" });
  const firestore = await firestoreClient(env); const current = await getAccountBalanceDoc(firestore, accountId);
  if (!current) return jsonResponse(404, { ok: false, error: "Account balance not found" });
  const updated = await updateCollectionDoc(firestore, "account_balances", accountDocId(accountId), { balanceStatus, updatedAt: new Date().toISOString() });
  await writeAuditLog(firestore, "admin", `balance_${balanceStatus}`, accountId, current, updated, String(payload.reason || "").slice(0, 500));
  return jsonResponse(200, { ok: true, accountId, balanceStatus });
}

async function adminReverseBalanceTopup(request, env) {
  if (request.method !== "POST") return jsonResponse(405, { ok: false, error: "Method not allowed" });
  const payload = await readJson(request); const auth = requireAdmin(payload, env); if (auth) return auth;
...
function buildOpportunityReviewRow(opportunityId, decision, payload) {
  const selectedChannel = cleanOptionalText(payload.selectedChannel || payload.selected_channel, 40);
  const publishedUrl = cleanOptionalText(payload.publishedUrl || payload.published_url, 500);
  const notes = cleanOptionalText(payload.notes, 1200);
  const feedbackLabel = normalizeFeedbackLabel(payload.feedbackLabel || payload.feedback_label);
  const hasReviewData = selectedChannel || publishedUrl || notes || feedbackLabel || ["reviewed", "approved", "rejected", "used"].includes(decision);
...
function normalizeFeedbackLabel(value) {
  ...
  return ["good", "neutral", "bad"].includes(text) ? text : "";
}
...
async function createAccountSession(accountId, env) {
  if (!env.ACCOUNT_SESSION_SECRET) throw new Error("ACCOUNT_SESSION_SECRET is not configured");
  const expiresAt = Date.now() + 24 * 60 * 60 * 1000;
  const payload = `${normalizeAccountId(accountId)}.${expiresAt}`;
  return `${payload}.${await hmacHex(payload, env.ACCOUNT_SESSION_SECRET)}`;
}

async function requireAccountSession(request, env) {
  if (!env.ACCOUNT_SESSION_SECRET) return jsonResponse(500, { ok: false, error: "ACCOUNT_SESSION_SECRET is not configured" });
  const token = String(request.headers.get("authorization") || "").replace(/^Bearer\s+/i, "");
  const [accountId, expiresAt, signature] = token.split(".");
  const valid = /^\d{10}$/.test(accountId || "") && Number(expiresAt) > Date.now() && signature && safeEqual(signature, await hmacHex(`${accountId}.${expiresAt}`, env.ACCOUNT_SESSION_SECRET));
  return valid ? accountId : jsonResponse(401, { ok: false, error: "Please log in again to use BestDia Balance" });
}
...
async function commitOrderAndAccountUpdates(firestore, orderDocId, orderUpdates, accountId, accountUpdates, ledger = null) {
  ...
  if (ledger) writes.push(firestoreCreateWrite(`projects/${firestore.projectId}/databases/(default)/documents/balance_ledger/${encodeURIComponent(ledger.transactionId)}`, ledger));
...
// A deterministic ledger ID with an `exists: false` precondition is the idempotency key:
// Firestore commits all writes or none, so a second Verify & Credit cannot create a second credit.
async function commitBalanceTopupVerification(firestore, topupDocId, topup, accountId, account, ledger) {
  const root = `projects/${firestore.projectId}/databases/(default)/documents`;
  const response = await fetch(`https://firestore.googleapis.com/v1/projects/${firestore.projectId}/databases/(default)/documents:commit`, {
    method: "POST", headers: { Authorization: `Bearer ${firestore.token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ writes: [
      firestoreUpdateWrite(`${root}/balance_topups/${encodeURIComponent(topupDocId)}`, topup),
      firestoreUpdateWrite(`${root}/account_balances/${accountDocId(accountId)}`, account),
      firestoreCreateWrite(`${root}/balance_ledger/${encodeURIComponent(ledger.transactionId)}`, ledger),
    ] }),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.error?.message || "Could not commit payment verification");
}

async function writeAuditLog(firestore, adminId, action, targetId, previousState, newState, reason) {
  return createCollectionDoc(firestore, "admin_audit_logs", {
    adminId, action, targetId, previousState, newState, reason: reason || "", createdAt: new Date().toISOString(),
  });
}

function firestoreCreateWrite(name, value) {
  const fields = {};
