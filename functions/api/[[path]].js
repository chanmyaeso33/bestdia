...
async function adminOrders(request, env) {
  ...
  const repairedOrders = await Promise.all(activeOrders.map(async (order) => {
    ...
    if (isBestDiaBalance && mxStatus === "submitted" && shouldAutoCompleteSubmittedMx(data, env)) {
      const now = new Date().toISOString();
      const timeline = Array.isArray(data.timeline) ? [...data.timeline] : [];
      timeline.push({ status: "completed", time: Date.now() });
      const updated = await updateOrderDoc(firestore, order.docId, {
        ...
        mxshopTopup: {
          ...
          autoCompleted: true,
          autoCompletedAt: now,
          reason: data.mxshopTopup?.reason || `Auto-completed after MXShop submitted grace period`,
        },
        updatedAt: now,
        timeline,
      });
...
function shouldAutoCompleteSubmittedMx(order, env = {}) {
  ...
  if (topup.status !== "submitted") return false;
  ...
  const submittedAt = topup.submittedAt || topup.completedAt || order.updatedAt || order.createdAt;
